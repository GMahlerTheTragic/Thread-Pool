...
/* Compress input to algorithm into a single argument */
typedef struct QSArgs
{
	size_t len;
	int *list;
} QSArgs;

static TASK(int, quicksort_parallel, QSArgs);

/* Define in analogy to fib. Note that the function cannot return void because of the macro */
static int quicksort_parallel(QSArgs args)
{
	if (args.len <= 1)
		return 0;

	size_t mid = partition(args.list, args.len);
	quicksort_parallel_fut *left = quicksort_parallelAsync((quicksort_parallel_fut[]){quicksort_parallelFuture((QSArgs){.len = mid, .list = args.list})});
	quicksort_parallel_fut *right = quicksort_parallelAsync((quicksort_parallel_fut[]){quicksort_parallelFuture((QSArgs){.len = args.len - mid - 1, .list = args.list + mid + 1})});
	quicksort_parallelAwait(left);
	quicksort_parallelAwait(right);
}

/* Keep original QS as benchmark */
...
{
	// Pool count changed to 2 because I only have 2 cores on my VM
	if (tpInit(2) != 0)
		perror("Thread Pool initialization failed"), exit(-1);
	atexit(&tpRelease);

	/* read the length of the list from the command line */
	// Default change to 10,000
	/* WICHTIG!!!!!: GGF STACK SIZE MANUELL MIT ulimt -s hard  ERHOHEN*/
	size_t list_len = 10000;
	if (argc >= 2)
	{
	...
	// Parallel implementation - Randomize again
	list_randomize(list, list_len);
	printf("Len: %lu\n", list_len);
	...
	/* sort it using the parallel algorithm and print the sorted list if it's not too long */
	clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &start);
	quicksort_parallel((QSArgs){.len = list_len, .list = list});
	clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &end);
	dt = time_diff(start, end);
	printf("---- Sortiert:   %i ----\n", list_is_sorted(list, list_len));
